...
def build_vocab(df_ids, new_file, vocab):
    ...
    for index, id in enumerate(df_ids):
        caption = str(new_file.loc[new_file['id']==id]['clean_caption'])
...
class COCOImagesDataset(Dataset):
    """ COCO custom dataset to read image data only,
        compatible with torch.utils.data.DataLoader. """
    def __init__(self, df, transform=None):
        """ Set the path for images, captions and vocabulary wrapper.
        
        Args:
            df (str list): dataframe containing all image information
            transform: image transformer
        """
        self.df = df
        self.IMAGE_DIR = "./coco/images/"
